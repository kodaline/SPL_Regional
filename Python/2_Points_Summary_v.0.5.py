...
def calculate_cumulative_points_and_rank(points_df):
    ...
    last_game_date = data_copy['Date'].max()
    # Calculate the rank for each player on each date
    ...
    # Calculate the rank change for each player
    data_sorted['Rank Change'] = data_sorted.groupby('Player')['Rank'].diff().fillna(0)
    # Filter the data to keep only the records corresponding to the last game date
    last_game_data = data_sorted[data_sorted['Date'] == last_game_date]
    # Extract the latest rank change for each player
    latest_rank_change = last_game_data['Rank Change']
    latest_rank_change = last_game_data.set_index('Player')['Rank Change']
...
def generate_summary(points_df):
    ...
    player_summary = pd.merge(player_summary, latest_rank_change.rename('Rank Change'), left_index=True, right_index=True, how='left')
